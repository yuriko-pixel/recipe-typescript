...
export const getRecipeByName = async (cusine: string): Promise<Recipe[]> => {
    const data = await (await fetch(`https://api.spoonacular.com/recipes/complexSearch?query=${cusine}&apiKey=${authKey}`)).json();
    return data.results.map((recipe: Recipe) => ({
        ...recipe
    }))
}

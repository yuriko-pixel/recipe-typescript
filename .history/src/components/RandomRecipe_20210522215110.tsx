...
export const getRecipeByIngredients = async (): Promise<DetailRecipe[]> => {
    const data = await (await fetch(`?apiKey=${authKey}`)).json();
    return data.results.map((recipe: Recipe) => ({
        ...recipe
    }))
}
...
export const getRecipeById = async (): Promise<Recipe[]> => {
    const data = await (await fetch(`?apiKey=${authKey}`)).json();
    return data.results.map((recipe: Recipe) => ({
        ...recipe
    }))
